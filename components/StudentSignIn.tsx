...
const StudentSignIn = () => {
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.user) {
        // Redirect based on user type or to students dashboard
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { data, error } = await authClient.signIn.social({
        provider: "google",
        callbackURL: "/students", // Redirect to students dashboard after success
      });
        ...
        <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
          ...
          <div className="mt-6 text-center">
            <p className="text-gray-600 text-sm font-mono">
              ...
              <a href="/forstudents/signup" className="text-[#007BFF] hover:text-[#0B1E3F] font-semibold transition">
                Sign up here
              </a>
            </p>
          </div>
