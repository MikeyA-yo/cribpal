...
import { Menu, X, Home, PlusCircle, Settings } from "lucide-react";
      ...
      >
        ...
        <div className="hidden md:flex items-center px-8 pt-10 pb-6 border-b border-blue-100">
          <div className="flex items-center gap-2">
            <span className="inline-block w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center text-white font-bold text-lg shadow">C</span>
            <span className="font-extrabold text-2xl tracking-tight text-blue-800">CribPal</span>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-1">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="flex items-center px-4 py-3 rounded-lg font-medium text-gray-700 hover:bg-blue-100 hover:text-blue-900 transition group focus:bg-blue-200 focus:outline-none active:bg-blue-200"
                onClick={() => setOpen(false)}
                style={{ marginBottom: 2 }}
              >
                <span className="group-hover:text-blue-700 mr-2">{item.icon}</span>
        ...
        <div className="w-full px-8 pb-6 pt-4 mt-auto hidden md:block">
          <div className="text-xs text-blue-400 text-center tracking-wide font-semibold opacity-90">Studio Manager Dashboard</div>
        </div>
